...
public class GetCode extends ActionSupport implements status{
    ...
    public String execute() throws Exception {
        ...
        String querySql = "select user_email from user_info where user_email='" + email
                + "'";
        ...
        if (rs.next()) {
            ...
            code = "";
            for (int i = 0; i < 6; i++) {
                code = code + new java.util.Random().nextInt(10);
            }
            System.out.println(code);
            ServletActionContext.getRequest().getSession().setAttribute("code", code);
            querySql = "select email from email_code where email='" + email
                    + "'";

            rs = con.createStatement().executeQuery(querySql);
            if (rs.next()){
                String updateSql="update email_code set code= '"+code+"' where email = '"+email+"'";
                con.createStatement().execute(updateSql);
            }
            else {
                String updateSql="INSERT  INTO email_code  (email,code) VALUE ( '"+email+"','"+code+"')";
                con.createStatement().execute(updateSql);
            }
            mail(email, code);
            STATUS=SUCCESSFUL;
    ...
    public void mail(String email,String code){
        // 发件人的 邮箱 和 密码（替换为自己的邮箱和密码）
        // PS: 某些邮箱服务器为了增加邮箱本身密码的安全性，给 SMTP 客户端设置了独立密码（有的邮箱称为“授权码”）,
        //     对于开启了独立密码的邮箱, 这里的邮箱密码必需使用这个独立密码（授权码）。
          String myEmailAccount = "supports@mail.enchant-music.cf";
          String myEmailPassword = "76307305202aC";

        // 发件人邮箱的 SMTP 服务器地址, 必须准确, 不同邮件服务器地址不同, 一般(只是一般, 绝非绝对)格式为: smtp.xxx.com
        // 网易163邮箱的 SMTP 服务器地址为: smtp.163.com
         String myEmailSMTPHost = "enchant-music.cf";
    ...
    public static MimeMessage createMimeMessage(Session session, String sendMail, String receiveMail,String code) throws Exception {
        ...
        // 2. From: 发件人
        message.setFrom(new InternetAddress(sendMail, "EnchantMusic", "UTF-8"));
